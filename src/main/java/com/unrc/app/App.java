        ...
        try {
            ...
            while (op == 1) {  // mientras sea opcion 1, 2 o 3
		      ...
		      //int depth = op * 3 + 1;	// profundidad por nivel: 4(facil), 7(medio) y 10(dificil)
                ...
                } while ( (!bc.isTheVictor(player)) && (turn < (numRow * numCol)) ); // opcional pero mas costoso: ( (!bc.isTheVictor(player)) || (!bc.fullBoard()) )
                System.out.println(b.toString());
                              //(!p.end(st));
                ...
                if (turn >= (numRow * numCol)) {  // empate o ganó el jugador #2 en la última jugada
